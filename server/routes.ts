...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/client/stage-invoices/:stageId/download-resimple", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      doc.text(`Tipo de cambio: 1 USD = ${exchangeRate.toLocaleString('es-PY')}`, leftMargin + 70, yPos + 27);
      doc.text(`${amountUSD.toFixed(2)} USD`, leftMargin + 370, yPos + 8);
      doc.text(`${amountPYG.toLocaleString('es-PY')}`, leftMargin + 370, yPos + 20);
      doc.text(`${amountUSD.toFixed(2)} USD`, leftMargin + 465, yPos + 8);
      doc.text(`${amountPYG.toLocaleString('es-PY')} PYG`, leftMargin + 465, yPos + 20);
      ...
      yPos += 12;
      doc.text(`Tipo de cambio: 1 USD = ₲ ${exchangeRate.toLocaleString('es-PY')}`, leftMargin, yPos);
      yPos += 12;
      doc.text(`Monto en guaraníes: ${amountPYG.toLocaleString('es-PY')} PYG`, leftMargin, yPos);
      yPos += 25;
